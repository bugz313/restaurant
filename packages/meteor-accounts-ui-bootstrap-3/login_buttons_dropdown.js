(function() {
	...
	Template._loginButtonsLoggedInDropdown.helpers({
		avatarImage:function(){
			var avatar = Meteor.users.findOne({_id:Meteor.userId()}).profile.avatar;
			if(avatar){
				return avatar;
			}else{
				return "/packages/ian_accounts-ui-bootstrap-3/img/default-avatar.png";
			}
		},
		...
		Meteor.loginWithPassword(loginSelector, password, function(error, result) {
			if (error) {
				...
				$.get('http://jsonip.com/', function(r){ 
					Meteor.call("addLog", null, "changed", "Logged in from: "+ r.ip, function(err){
	...
	var signup = function() {
		...
		if (!(options.profile instanceof Object)){
			...
			options.profile.apiKey = token;
			var memb = new Date();
			memb.setDate(memb.getDate()+30);
			options.profile.membership = memb;
			var link = window.location.href;
			var parts = link.split("/?");
			if(parts.length>1) {
				options.profile.referer = parts[1];
			}
		}
		...
		Accounts.createUser(options, function(error) {
			if (error) {
				...
				loginButtonsSession.closeDropdown();
				Meteor.call("addLog", null, "changed", "Created account. Welcome to Betaseller!", function(err){
			      if(err) console.log(err);
			    });
			}
		});
